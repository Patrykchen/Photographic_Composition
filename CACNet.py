...
class CompositionModel(nn.Module):  # 图像构图模型及参数初始化
    def __init__(self):
        super(CompositionModel, self).__init__()
        self.comp_types = 9
        backbone = models.densenet201('pre_checkpoints/densenet201-c1103571.pth')
        features = list(backbone.features.children())
        self.body = nn.Sequential(*features[:12])
        self.avgpool = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Flatten(1))
        self.fc_layer = nn.Linear(1920, self.comp_types, bias=True)
        # self.classifier = nn.Softmax()

    def forward(self, im):
        x = self.body(im)
        gap = self.avgpool(x)
        logits = self.fc_layer(gap)  # output
...
class CACNet(nn.Module):
    ...
    def forward(self, im, only_classify=False):
        ...
        logits,kcm = self.composition_module(f2,f3,f4)
        # logits,kcm = self.composition_module(im)
